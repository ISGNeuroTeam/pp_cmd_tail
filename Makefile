...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pp_cmd_tail_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pp_cmd_tail-*.tar.gz
	echo Create archive \"pp_cmd_tail-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_tail-$(VERSION)-$(BRANCH).tar.gz tail

clean_pack:
	rm -f pp_cmd_tail-*.tar.gz


pp_cmd_tail.tar.gz: build
	cd make_build; tar czf ../pp_cmd_tail.tar.gz tail && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./tail make_build
	cp *.md make_build/tail/
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/tail

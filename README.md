# pp_cmd_tail
Postprocessing command "tail"
## Description
tail do (write here what it does)  
...
```
... | tail positional_arg, kwarg_int_argument=4  ...
```
...
### Run tail
Use `pp` to run tail command:  
```bash
pp
Storage directory is /tmp/pp_cmd_test/storage
Commmands directory is /tmp/pp_cmd_test/pp_cmd
query: | otl_v1 <# makeresults count=100 #> |  tail 
```
## Deploy
1. Unpack archive `pp_cmd_tail` to postprocessing commands directory
